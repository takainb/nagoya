...
public class CategoryRestaurantService {
	...
	@Transactional
	public void update(List<Integer> categoryIds, Restaurant restaurant) {
		...
		if (categoryIds == null) {
			// categoryIdsがnullの場合は全てのエンティティを削除する
			...
			for (CategoryRestaurant existingCategoryRestaurant : existingCategoryRestaurants) {
				if (!categoryIds.contains(existingCategoryRestaurant.getCategory().getId())) {
					categoryRestaurantRepository.delete(existingCategoryRestaurant);
				}
			}

			// 新しいIDが既存のエンティティに存在しない場合は新たにエンティティを作成する
			for (Integer newCategoryId : categoryIds) {
